...
class Renderer {
    ...
    renderUI(data) {
        // data = { gameState, tanks, projectiles, explosions, deltaTime }

        // Render round info and timer
        this.renderRoundInfo(data.gameState);
        ...
        if (window.debugMode && window.debugMode.stats) {
            this.renderMatterStats(data);
        }

        // Render velocity vectors
        if (window.debugMode && window.debugMode.velocityVectors) {
            this.renderVelocityVectors(data.tanks || []);
        }
    }
    ...
    renderMatterStats(gameState) {
        ...
        // Background box (키워서 FPS 공간 확보)
        this.ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
        this.ctx.fillRect(x - 10, y - 20, 290, 280);
        ...
        if (fps < 50) fpsColor = '#ffaa00'; // Orange
        if (fps < 30) fpsColor = '#ff0000'; // Red

        this.ctx.fillStyle = fpsColor;
        this.ctx.font = 'bold 14px monospace';
        this.ctx.fillText(`FPS: ${fps}`, x, currentY);
        currentY += lineHeight * 1.5;
