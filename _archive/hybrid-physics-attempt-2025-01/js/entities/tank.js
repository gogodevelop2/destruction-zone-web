...
class Tank {
    ...
    static FORCE_SCALE = 0.00015;      // Matter.js 힘 스케일
    ...
    constructor(options) {
        ...
        if (this.physics && typeof Matter !== 'undefined') {
            try {
                ...
                console.error(`❌ Failed to create Matter.js body for ${this.id}:`, error);
                console.log(`⚠️ ${this.id} will use original physics`);
            }
        } else {
            console.log(`ℹ️ ${this.id} using original physics (Matter.js not available)`);
        }
    ...
    // ========================================
    // Matter.js v2: Physics Body Creation (원형 바디 - 안정적)
    // ========================================
    createPhysicsBody() {
        const size = this.stats.size * 0.8;

        // 원형 바디 (무게중심 = 원점, 가장 안정적)
        ...
        console.log(`🔧 Tank ${this.id} Matter.js circle body created: radius=${(size * 0.65).toFixed(1)}, mass=${this.body.mass.toFixed(1)}`);
    }
    ...
    update(deltaTime) {
        ...
        if (this.body && this.physics && this.physics.engine) {
            ...
            try {
                ...
                console.error(`❌ Matter.js physics error for ${this.id}:`, error);
    ...
    updateWithMatterPhysics(deltaTime) {
        ...
        // Matter.js 바디 위치 → 렌더링 위치
        this.x = body.position.x;
        this.y = body.position.y;
        this.velocity = { ...body.velocity };

        // 각도는 우리가 직접 계산했으므로 물리 바디에 반영
        Matter.Body.setAngle(body, this.angle);
        // angularVelocity는 우리가 관리하므로 Matter.js 값 무시
        ...
        if (Math.random() < 0.01) {
            ...
            if (offset > 1) {
                console.log(`⚠️ ${this.id} position offset: ${offset.toFixed(2)}px`);
            }
        }
    }
